...
export type ColourInputs = {
  ...
  /** The TTY-ness of the stream being drawn to. Absent is not a TTY: a stream
   * that says nothing about itself is the redirected case, and reading it as
   * unknown would emit different bytes. */
...
export function coloursEnabled({ env, isTTY, request }: ColourInputs): boolean {
  // Bound up front only because the environment is an index signature. The
  // order below is what decides; binding a name reads nothing that deciding
  // early would have had to unread.
