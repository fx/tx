...
- **A version manager owns it.** mise and npm are recognized from the executable's own resolved location, and from `MISE_DATA_DIR` or `MISE_INSTALLS_DIR` when either moves mise's store off the path that would announce it. The manager is asked which tool owns that path and its own upgrade command is run for that tool — `mise upgrade <tool>` or `npm install --global <package>` — and its output is reported. That one mise command runs with `MISE_MINIMUM_RELEASE_AGE=0`, so mise does not withhold a release `tx` has already offered; your configuration is untouched and every other tool keeps the policy. Afterwards the manager is asked what it has installed, and the item is reported as applied only against a version newer than the one that was running that the manager was not already reporting before the upgrade — mise lists every version it has installed, so a newer one you have pinned away from is not something this upgrade did. An upgrade that exits successfully having installed nothing is reported as nothing applied, with the manager's output and the version still installed. Nothing inside the manager's store is replaced by `tx`, because a manager that kept recording the old version would silently revert the update on its next install. A recognized manager that cannot be interrogated is a failure, not a reason to write into its store anyway.
...
A consumer reads `registrations<Config>("@fx/tx/config")` inside its command action, after initialization has committed every provider. It must find **exactly one**, by the same rule `requireConfigCapability` applies for the marketplace plugin: none and several are both errors naming the count, and there is deliberately no fallback. The config plugin is composed by default, so a `tx` without it is misconfigured rather than degraded.
...
Up and Down move one position among the visible options and clamp at the first and last of them. Home and End make the first and the last visible option active, and Page Up and Page Down move by the number of option rows on screen, clamped the same way. Enter on a plain option resolves `{ value, values }`, where `value` is that option's exact value and `values` is empty. Escape and Ctrl-C return `undefined` at every stage; the provider does not terminate the process, assign an exit code, or print the selected value. Input the dialog has no meaning for is ignored; a printable character is never one of them, because typing always reaches the filter.

An option may declare a sub-dialog under `dialog`: either a nested select request (its own message, options, and optional filter) or a single text field. An option that declares one is marked with `▸` on the right edge of its column, and opening it adds a column to the right of the one it was opened from rather than a panel over it. Enter opens the marked option by default — the key that takes a plain option opens one that leads somewhere, which is what the marker promises — and the right arrow opens under any binding. The root request's `expand` field rebinds opening to `"tab"`, which keeps Enter meaning "take this" at every level; it is read from the root request only, so one dialog answers one set of keys however deep the reader goes. Without a declaration every one of those keys is a no-op on that row and the filter text is left untouched, and typed or pasted text never opens anything. The left arrow backs out into the column that opened this one and does nothing at the leftmost column, where Escape rather than an arrow is what closes the dialog. Escape pops one column while anything is open and cancels only at the root; Ctrl-C does the same. Every column keeps its own filter text and active position, so opening one starts it blank on its first option while the ones behind it keep what they were showing, and backing out restores exactly what they showed. Completing any column resolves the whole path: the finally-Entered option's value plus every input value collected along the way, merged by field name with deeper submissions winning collisions. A text leaf resolves with its opening option's value and its submitted text under its field's name. Every reachable sub-request is validated before rendering alongside the existing rejections — an empty nested list, an empty field list, or a repeated field name within one nested option renders nothing. The hint line names exactly the keys the dialog answers where the reader is: at the leftmost column, `↑↓ move · →/Enter open · type to filter · Esc cancel` on a row that leads somewhere and `↑↓ move · Enter select · type to filter · Esc cancel` on one that does not; once a column has been opened, `←/Esc back` joins the line and `Esc cancel` leaves it, because Escape backs out of a column rather than cancelling the dialog. Every column is still one render session with the same cleanup contract.

The columns render inside one frame — the first level's — in greyscale only. A sub-dialog has no border, offset, or shadow of its own, so a path three levels deep is still one bordered panel with three lists side by side, separated by a dimmed `│`. The columns share one band of rows — nothing but option rows is in it, since the filter and the counts live in the frame's edges — so a list of three and a list of thirty start on the same row, and a column whose filter matched nothing spends its one row saying `no match`. Every column draws the list it was showing with its cursor bar still on the choice it was left on, and every bar is drawn the same way — the inversion alone, with no shading to set the driven column's apart. Which column is being driven is said by where it sits: it is the rightmost. A column behind the driven one is dressed at rest: nothing on it animates, and its own filter text and hidden counts are not drawn, because the edges that carry those belong to the column taking keys. The panel's title is the trail of the columns on screen, joined with `›`. When the columns run out of room the leftmost is dropped first, and the title says so with a leading `…`; collapsing stops at one column, which is truncated to what is left rather than dropped. A text leaf's entry renders as its own panel beneath the frame, exactly as a collected field does.

Typing always filters. A printable character narrows the column the reader is in, at every level and whatever the list's length; there is no setting that makes typing do nothing. `filter` decides only whether the filter is on screen before anything has been typed into it: `"typed"`, the default, keeps it off screen until it is used, and `"always"` shows it from the moment the dialog opens. Because it is set into the frame's bottom edge rather than drawn as a row, it appearing costs the panel no row and moves nothing in the list.

The filter is the element typed text reaches from the moment the dialog opens; no key moves focus to or from it, and the arrows, Enter, Escape, and Ctrl-C keep the meanings they have without it. Printable characters and Backspace edit the filter text exactly as they edit an `input` value, and the current text is rendered after a `›` prompt. The text's whitespace-separated pieces are its terms: an option is visible when every term occurs in its label under a case-insensitive comparison, so `rel 1.4` finds `release/1.4` in any term order. Matching is substring only — there is no fuzzy matching, ranking, match highlighting, matching against values, caller-supplied matcher, or initial filter text — so visible options keep the order and the duplicates the caller supplied. An option that declares `fields` is the caller's "let me type it" answer and stays visible whatever the text, since typing something nothing matches is exactly when it is needed. Changing the text makes the first visible option active. When nothing is visible the dialog renders `no match`, Enter and navigation do nothing, and Escape still cancels — Escape never clears the filter instead. Field collection stops filter edits along with navigation, and the filter text never appears in the result.

Every `select` renders its options through a viewport, filtered or not, so a long list neither overflows the terminal nor makes rendering cost grow with the option count. At most ten option rows appear at once, and fewer when the terminal is short: the window shrinks so that the whole dialog — its two frame edges and its key hints — stays shorter than the terminal, which is what keeps it from clearing or scrolling away what was on screen before it opened. Choosing a user-provided option puts that field's own frame on screen underneath, so the window gives rows back on the frame that first draws it rather than reserving them the whole time; a terminal too short for one option row once the field arrives simply shows none while the field is collected. In a terminal too short for even one option row plus those other rows, the window gives up its last row rather than fill the screen: the dialog keeps its edges and the counts set into them, still navigates, and still settles on a choice it can no longer draw, because a window the reader cannot see beats a terminal wiped out from under them. The window moves only as far as keeping the active option on screen needs, so the list stays still under the cursor bar rather than recentering on every keystroke, and it follows the terminal: a resize re-derives it. When visible options sit outside the window the dialog says so on that side and counts them, as `▲ N` set into the frame's top edge and `▼ N` into its bottom. They live on the edges rather than in rows of their own, so one appearing as the reader scrolls past the first hidden row moves nothing; the room for them is held from the first frame, so the title does not resize either. The window is over the visible options, so it composes with the filter: its rows and its hidden counts describe what the filter left, not the list the caller supplied.
...
Both dialogs are drawn the way Norton Commander drew its panels, in greyscale. Each is a framed panel carrying its message as a title set into the top edge — the request message for a `select` or a standalone `input`, the field's own message for a field under collection — with a `select` in a double-line frame and an `input` or a field in a single-line frame. The active option is an inverted bar spanning its column's width rather than a marker character — and the rightmost column takes whatever the title and the edges left the panel spare, so a select showing one column has its bar spanning the whole frame — and the frame edges, the title, the `›` filter prompt, the `▲ N` and `▼ N` overflow counts, and the key hint line are dimmed relative to option labels and entered text. That is the whole palette: the terminal's default foreground and background, their dimmed form, and their inversion. No hue is ever emitted, so a dialog looks the same on a light theme, a dark one, a sixteen-color terminal, and a true-color one, and there is nothing to configure.

A dimmed key hint line follows the lowest frame on screen — exactly one of them, however many columns are on screen: `↑↓ move · Enter select · type to filter · Esc cancel` under a `select`, with the phrases the current column and the row under the bar answer as described above, and `Enter submit · Esc cancel` under a standalone `input` or a field collected at the leftmost column, which is the line shown while such a field is collected because navigation and filtering have stopped by then. An entry opened over a column — a text-field sub-dialog, or a field of an option chosen in a column that was opened — reads `Enter submit · Esc back` instead, because its Escape closes that entry and returns to the column that opened it rather than cancelling the dialog. It is the same rule the select's own line follows, minus the left arrow an entry does not answer. A frame is as wide as the wider of its title and its widest row, and never wider than the terminal; twenty columns is the narrowest terminal it lays out for, and how a narrower one wraps the result is unspecified. Nothing wraps: a title or a label too wide for the frame is truncated at its end with an ellipsis, while filter text and a value under entry are truncated at their start so their tail and the `█` caret drawn after them stay in view. Wrapping is what the truncation avoids — a label spilling onto a second row would change the number of rows the viewport counted on.

Three things move, and nothing else does. The `█` caret after entered text and after the filter alternates between visible and hidden about twice a second while the dialog waits for input, keeping its cell either way so the panel never resizes on its own timer; typing puts it straight back on its visible phase, so a keystroke is never answered by a row that looks like it lost its caret. Only a line that still answers keystrokes carries one: the filter keeps its text and its `›` prompt once a field is being collected, but gives up its caret, because it has stopped accepting edits by then and the field below it is the row being typed into. An overflow indicator pulses between its dimmed and normal rendering on the same phase, and only while it is on screen at all — that is, only while the window is hiding rows. Confirming a plain option flashes the cursor bar before the dialog settles: the choice is recorded the moment Enter is pressed, every key from then on is ignored — Escape and Ctrl-C included, because the choice is already made — and the dialog settles well inside the 250 milliseconds the spec allows, on elapsed time rather than on any particular frame. Field collection is not confirmation and does not flash; it moves straight to the field. Neither does a confirmation in a terminal too short to draw an option row, since there is no cursor bar on screen to flash — it settles at once rather than spending the flash ignoring input over a frame that would not change.
