...
The SSH attempt runs `ssh -o BatchMode=yes` by default, so an unknown host key or a missing key fails rather than asking. If you already configure an SSH command — through `GIT_SSH_COMMAND`, through `GIT_SSH`, or through Git's own `core.sshCommand`, which is how a CI job usually pins a deploy key — that command is used exactly as it stands, and none of the three is overridden. Yours is a deliberate invocation, an identity file, an alternate config, a proxy command, and `tx` has no business rewriting it. That includes its prompting behavior: a key whose passphrase is not already in your agent can still stop and ask.
