...
- A ref carried as a suffix MUST NOT contain `/`. The separator rule is what makes the suffix unambiguous without parsing the source, and it cannot survive a ref that contains the character it keys on. A ref spelled with `/`, such as a `release/1.4` branch, MUST be reachable by adding the marketplace and then pinning it, and MUST be rejected as a suffix rather than silently read as part of the source.
...
- "Higher" MUST mean higher as a semantic version, and only tags that parse as one participate — on both sides. A pin to a tag that is not a semantic version reports no comparison at all, and tags that are not semantic versions are never reported as higher than anything. Creation time, lexical order, and reachability each order tags differently, and only one of the four answers the question a user asks about a release.
...
- An available version MUST be reported only when the published version is strictly newer *and* this executable can be updated at all. Where it cannot — a source checkout, or a platform with no published executable — the newer release MUST be reported as detail with no available version, so the user learns it exists and the driver never tries to apply what the participant would only refuse.
