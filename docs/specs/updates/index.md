...
- The ref MAY contain `/`, so `fx/cc@release/1.4` pins to that branch. Excluding the authority before searching is what makes that unambiguous, and it is the same boundary [Plugin System: Local Marketplace Sources](../plugin-system/index.md#local-marketplace-sources) already reads to decide that a source carries Git syntax.
