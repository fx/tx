...
[Change 0030](../../changes/0030-publish-bundled-capability-contracts.md) publishes this contract at `@fx/tx/theme` and renames the capability's registry key to that same specifier, so a consumer imports the contract rather than restating it, under [Plugin System: Published Capability Contracts](../plugin-system/index.md#published-capability-contracts). That change is approved and not yet implemented, so exactly two things below are ahead of the code: the contract is not published today, and the key in use is still the bare `theme`, as is the override key still registered as the bare `theme-override`. Every other requirement describes current behavior.
