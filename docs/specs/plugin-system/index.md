...
[Published Capability Contracts](#published-capability-contracts) is fully implemented. [Change 0030](../../changes/0030-publish-bundled-capability-contracts.md) established the mechanism and [Change 0031](../../changes/0031-publish-the-dialogs-and-config-contracts.md) finished the set, both complete: the [Theming](../theming/) contract is published at `@fx/tx/theme`, its override at `@fx/tx/theme-override`, the [Grid](../grid/) contract at `@fx/tx/grid`, the [Config](../config/) contract at `@fx/tx/config`, and the [Dialogs](../dialogs/) contract at `@fx/tx/dialogs`. Every bundled capability's registry key is the specifier its contract is published at, no key is bare, and every bundled consumer, the demo, and the capability tests import those contracts rather than restating them.
