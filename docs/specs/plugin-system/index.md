...
- A validation failure whose cause does not depend on which files are present — neither supported manifest location existing in the repository at all, invalid JSON, an empty or missing `plugins` array, or a field of the wrong shape — MUST be reported without first retrieving the complete tree, since retrieving more of the tree cannot change that outcome.
