...
- A validation failure whose cause does not depend on which files are present — neither supported manifest location existing in the repository at all, invalid JSON, an empty or missing `plugins` array, a field of the wrong shape or type, or an entry or package value that is empty, absolute, or lexically escapes the repository — MUST be reported without first retrieving the complete tree, since retrieving more of the tree cannot change that outcome.
