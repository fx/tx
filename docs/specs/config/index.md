...
[Change 0031](../../changes/0031-publish-the-dialogs-and-config-contracts.md) publishes this contract at `@fx/tx/config` and renames the capability's registry key to that same specifier, so a consumer imports the contract rather than restating it, under [Plugin System: Published Capability Contracts](../plugin-system/index.md#published-capability-contracts). That change is complete — publication, the key rename, and the move of the marketplace plugin, the capability tests, and [the plugin guide](../../manual/plugins.md) off their local copies of the shape — so every requirement below describes current behavior.
