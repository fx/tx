...
[Change 0030](../../changes/0030-publish-bundled-capability-contracts.md) publishes this contract at `@fx/tx/grid` and renames the capability's registry key to that same specifier, so a consumer imports the contract rather than restating it, under [Plugin System: Published Capability Contracts](../plugin-system/index.md#published-capability-contracts). That change has landed, so every requirement below describes current behavior.
...
- A cell declaring no `variable` MUST be drawn as `content`, the role [Theming: Theme Variables](../theming/index.md#theme-variables) gives a surface's own text. An absent `variable` names that role rather than leaving the cell's appearance for the implementation to choose, so the bare-string shorthand and a cell declaring `content` render identically. What an absent `align` means is [Table Layout](#table-layout)'s to state, and it states it.
...
- A row declaring no actions MUST resolve on the row alone, so a grid used only to pick something needs no action model. Its selection MUST carry no `action`, and an absent `action` on a selection MUST mean exactly that: the chosen row offered none. It MUST NOT be produced by a row that offered actions, because backing out of an actions column returns to the row list rather than selecting the row without one.
...
- Whether a selection SHOULD say that its chosen row offered no actions by something other than the absence of its `action` is undecided. The absence carries the meaning, and what the field holds when it is there is the consumer's own value — so nothing stops a consumer giving an action the very value a field that is not there reads as, and the two questions "is the field there" and "is what the field holds the absent value" then answer differently. They differ for that one consumer alone, which is what makes it hard to see: the second question is the one a reader asks by reflex, it agrees with the first everywhere else, and the worked example this specification's guide carries was written that way and read four times before a reader looking at the example on its own noticed. A contract that does not survive its own example is not one a consumer will hold. Two shapes would close it. A selection could name which of the two answers it is in a field every selection carries, whose values are this specification's rather than any consumer's — which distinguishes the two without reading `action` at all, and lets a reader who has established which case they are in be certain the action is there — at the cost of a longer reading in the commoner case, the one where an action was chosen. Or an action's value could be constrained so that it can never be the value an absent field reads as, which removes the case rather than making it representable and costs a consumer only a value it was unlikely to want. Reporting a separate flag saying whether the row offered actions is rejected rather than left open: it would be a second thing that can disagree with the first, which is the fault it was meant to cure. What bounds all of it today is that the disagreement is reachable only for a consumer that chooses that value for an action, and the capability has no consumer at all yet; the first real one would settle both which shape is worth its cost and whether the cost is worth paying.
...
- Whether the grid SHOULD measure display width with the same implementation the renderer measures it with is undecided. They are not the same one today: the grid asks the runtime, the renderer carries its own, and the renderer exposes nothing to ask — so agreeing with it would mean importing a width library, which is the dependency [Cell Values](#cell-values) deliberately does without. What bounds it is that the grid's own alignment is internally consistent, every width in it coming from one measure, and that the width the grid hands the renderer only bounds where a line may wrap. A character the two disagree on could still put a line past that bound and have it wrapped, which would cost [Printing](#printing) its identical-bytes guarantee for that one input. A consumer meeting such a character, or a renderer that exposes its measure, would settle it.
