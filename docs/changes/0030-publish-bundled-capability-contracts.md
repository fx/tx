...
- `@fx/tx/theme-override` gets a published subpath of its own like every other key, carrying the shape of the value registered under it and declaring it in exactly one module. It is not a capability, which changes nothing: [Plugin System: Published Capability Contracts](../specs/plugin-system/index.md#published-capability-contracts) states the rule over keys, so a key carrying contributions to a capability is published exactly as one carrying a capability — the plugin registering an override has to type what it registers.
...
One contract module per published key at `plugins/<name>/`, containing `export type` declarations and nothing else, imported type-only by its own provider so the value the provider registers is checked against the contract it publishes. The unit is the key rather than the capability, which is why `theme-override` gets a module and a subpath of its own alongside `theme`. `package.json` gains a types-only `exports` entry per key and the matching `files` entries. `test/plugin-boundary.test.ts` generalizes its `@fx/tx/plugin` rules over the published subpath set.
...
- [ ] Whether the published contracts should eventually be emitted `.d.ts` files rather than raw TypeScript. Raw source is what `./plugin` already publishes and what the consumer fixture proves, so this change follows it; the cost is that every consumer needs `moduleResolution` set to a mode that reads an `exports` map, which was a requirement no document stated. [The plugin guide](../manual/plugins.md) now states it, along with the matching `module` setting and ES-module importer that `node16` and `nodenext` additionally need. `allowImportingTsExtensions` turned out not to be part of that cost: TypeScript follows the contracts' internal `.ts` imports without it, so a consumer that emits its own JavaScript or declarations is not excluded. Whether to remove the resolution requirement altogether is separate and affects `./plugin` equally.
