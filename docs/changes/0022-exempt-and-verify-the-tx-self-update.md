...
Make `tx update` tell the truth about updating `tx` itself. The delegated `mise upgrade` runs with mise's minimum release age disabled for that one command, so the release `tx` just offered is not withheld from the command sent to install it; and a delegated upgrade counts as applied only when the manager afterwards reports a newer version it was not already reporting, so an upgrade that changed nothing is reported as nothing applied rather than as success. [Updates: Executable Updates](../specs/updates/index.md#executable-updates) owns both behaviors.
...
- **The post-upgrade observation reuses the listing the delegation already reads.** `Installation` gains the version each manager's listing already reports — mise's JSON entries carry it beside `install_path`, and npm's parseable listing carries it in the `<name>@<version>` label the reader already splits. The interrogation that asks which tool owns the path already reads that listing, so the versions reported before the upgrade come out of it for free; re-reading it afterwards and looking for a version that appeared and is newer than the one running is the whole check. Among several such versions the newest that can be ordered at all decides, so an entry nothing can order — mise lists `ref:` installs beside released versions — cannot hide the ones that can.
...
`#delegate` grows one step at each end and keeps its shape. Before running the upgrade it builds the child's environment from the injected `env` plus whatever the owning manager declares its upgrade needs. It also has the state before the upgrade in hand: the interrogation that names the owning tool reads the manager's listing, and the versions it reports for that name come back with the name. After the upgrade exits zero it re-runs that listing, takes the versions the manager now reports for the tool it just upgraded, and asks whether any of them is one that was not there before and is strictly newer than the version that was running.

Both halves of that question matter. A manager may list every version it has installed, so comparing only against the running version would read a newer version that was already on disk — a tool the user pinned to an older one — as something this upgrade installed. And where several versions are candidates, the newest is chosen among those that can be ordered at all rather than folded through one accumulator: `isNewerRelease` refuses a comparison either side of which it cannot parse, so a single unorderable entry would otherwise latch the answer and let the order the manager listed its installs in decide the report. A listing whose versions can none of them be ordered remains evidence of movement in neither direction.
...
- **Decision:** Decide "did it move?" from the manager's own listing after the upgrade, requiring a version that was not in the same manager's listing before it and that is newer than the version running.
  - **Why:** It is the only observation that works for both managers with one mechanism, it reuses the reader and the ordering the file already has, and it asks the component whose record delegation exists to keep true. Running the executable is not an option: the resolved target is the old version's install path under mise, so it would report the old version on success or fail on success. Comparing against the running version rather than against the offered one keeps a manager that installs some other newer version — a release GitHub has not marked latest — reported as the update it is, and names the version actually installed. The before-and-after comparison is what makes that honest under mise, which lists every installed version: a newer one the user has pinned away from is reported by the listing without this upgrade having installed anything. Asking which version *appeared*, rather than requiring the newest reported to have moved, still reports correctly when the manager installs something while an even newer version sits pinned beside it, and the baseline is free — the interrogation that names the owning tool has already read that listing.
  - **Alternatives considered:** Requiring the manager to report exactly the offered version was rejected as calling a real upgrade a failure whenever the manager's idea of latest differs from GitHub's. Requiring only the newest reported version to be newer than the running one was rejected as the pinned case above: it reports success for an install that was there all along. Diffing install paths was rejected because npm installs into the same global prefix and its path never changes. Parsing the upgrade's output was rejected as reading a message with no contract behind it. Re-running the executable was rejected as above.
...
- [x] Report a delegated upgrade against what the manager installed
  ...
  - [x] Carry the versions the owning manager already reports for the tool out of the interrogation that reads its listing, as the state before the upgrade
  - [x] Re-read that listing after a zero-exit upgrade and take the newest orderable version it now reports that it did not report before, independently of the order it lists them in
...
- [x] Cover the new behavior in `test/executable-plugin.test.ts`
  ...
  - [x] A post-upgrade listing that cannot be read, and one naming only unorderable versions
  - [x] An unorderable version listed either side of a newer one, and a newer version the manager reported before the upgrade as well as after it
