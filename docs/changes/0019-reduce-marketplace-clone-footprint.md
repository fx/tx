...
- **Footprint planning and final validation are two distinct passes over the manifest, and only the second may touch the filesystem.** `resolveMarketplaceManifest`'s existing shape checks — that the document is an object, `plugins` is a non-empty array, each entry has a safe unique `name`, a string `entry`, and a well-typed `package` — and `resolvePluginEntry`'s and package-candidate selection's existing *lexical* checks — non-empty, not absolute, lexically contained by the repository, and, for an explicit `package`, named `package.json` exactly (`basename`, no filesystem access) — none of which require a path to exist — do not depend on any path existing, and `storage.ts` gains an internal step that performs only those shape and lexical checks and returns the raw repository-relative `entry`/`package` strings for whatever survives them, with no call into the existence, regular-file, or realpath-containment resolution `resolvePluginEntry` and package-candidate selection also perform. The reduced-retrieval planner calls that step to compute the directories to fetch (`dirname` of each `entry` and each explicit `package`, or the entry's directory when `package` is omitted) *before* those paths are guaranteed to exist; an `entry` or `package` that fails a shape or lexical check yields no directory and is reported immediately as a content-only failure, since no amount of additional fetching changes that outcome. **Existence, regular-file, and realpath-containment resolution stay exactly where they are today:** once the planned directories are materialized, the unmodified `resolveMarketplaceManifest` — `resolvePluginEntry` and package-candidate resolution included — is what actually accepts or rejects the manifest; the planner never re-derives or duplicates that judgment.
...
- **Decision:** Split `storage.ts`'s manifest handling into a syntax-only step the planner calls before anything beyond the manifest itself is fetched, and keep `resolveMarketplaceManifest`'s existing filesystem-touching resolution as the sole authority for the actual validation pass.
  - **Why:** `resolveMarketplaceManifest` resolves each entry's existence, regular-file status, and containment (via `resolvePluginEntry`) as part of producing its result, and `resolvePluginEntry` throws the moment a path does not exist. Calling it against a checkout that has only the manifest and not yet the directories a normal entry lives in — the common case, since most entries point outside `.tx/` — would throw immediately for essentially every real marketplace, before the planner ever learns which directories to fetch. The step the planner needs reads and validates the manifest's JSON structure and field types exactly as `resolveMarketplaceManifest` does today, **and** applies the same lexical checks `resolvePluginEntry` and package-candidate selection already apply before touching the filesystem — non-empty, not absolute, lexically contained by the repository (`resolve`/`relative`, no `stat` or `realpath`), and, for an explicit `package`, named `package.json` exactly (`basename`, storage.ts's `selectPackageCandidate`) — to each `entry` and any explicit `package`. Only a path that survives those checks yields a directory the planner can ask sparse-checkout for; one that fails them is exactly as invalid as `resolveMarketplaceManifest` would eventually find it, without needing a single file to exist, so it is reported immediately as a content-only failure rather than treated as something a complete retrieval might resolve. This step duplicates none of `resolveMarketplaceManifest`'s *filesystem-dependent* logic — existence, regular-file status, realpath containment — all of which stay in the one place that already owns them and runs unchanged for the actual validation pass this document's other decisions govern.
...
- [ ] Add manifest-driven reduced retrieval to marketplace install (PR #1)
  - [ ] Add a syntax-only manifest-planning step to `storage.ts` that validates JSON shape, field types, and the same lexical path rules `resolvePluginEntry`/package selection already apply (non-empty, not absolute, lexically contained, and an explicit `package` named `package.json` exactly), and returns each valid entry's raw `entry`/`package` strings without resolving them against the filesystem, sharing its rules with `resolveMarketplaceManifest` rather than duplicating them
