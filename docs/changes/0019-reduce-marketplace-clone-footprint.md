...
- **Footprint planning and final validation are two distinct passes over the manifest, and only the second may touch the filesystem.** `resolveMarketplaceManifest`'s existing shape checks — that the document is an object, `plugins` is a non-empty array, each entry has a safe unique `name`, a string `entry`, and a well-typed `package` — do not depend on any path existing, and `storage.ts` gains an internal step that performs only those checks and returns the raw repository-relative `entry`/`package` strings, with no call into `resolvePluginEntry` or package-candidate resolution. The reduced-retrieval planner calls that step to compute the directories to fetch (`dirname` of each `entry` and each explicit `package`, or the entry's directory when `package` is omitted) *before* those paths are guaranteed to exist. **Existence, regular-file, and containment resolution stay exactly where they are today:** once the planned directories are materialized, the unmodified `resolveMarketplaceManifest` — `resolvePluginEntry` and package-candidate resolution included — is what actually accepts or rejects the manifest; the planner never re-derives or duplicates that judgment.
...
- **Decision:** Split `storage.ts`'s manifest handling into a shape-only step the planner calls before anything beyond the manifest itself is fetched, and keep `resolveMarketplaceManifest`'s existing filesystem-touching resolution as the sole authority for the actual validation pass.
  - **Why:** `resolveMarketplaceManifest` resolves each entry's existence, regular-file status, and containment (via `resolvePluginEntry`) as part of producing its result, and `resolvePluginEntry` throws the moment a path does not exist. Calling it against a checkout that has only the manifest and not yet the directories a normal entry lives in — the common case, since most entries point outside `.tx/` — would throw immediately for essentially every real marketplace, before the planner ever learns which directories to fetch. A shape-only step that reads and validates the manifest's JSON structure and field types, and returns the raw `entry`/`package` strings without resolving them against the filesystem, is what the planner needs; it duplicates none of `resolveMarketplaceManifest`'s existence, regular-file, or containment logic, all of which stay in the one place that already owns them and runs unchanged for the actual validation pass this document's other decisions govern.
  - **Alternatives considered:** Having the planner call `resolveMarketplaceManifest` directly against the initial checkout was rejected once it was clear that throws before returning anything for the common case, which this document's earlier draft did not account for. Having the planner parse and shape-validate the manifest with its own independent logic, rather than a step `storage.ts` exports, was rejected as a second implementation of the shape rules `resolveMarketplaceManifest` already enforces, which could drift from it over time.
...
- [ ] Add manifest-driven reduced retrieval to marketplace install (PR #1)
  - [ ] Add a shape-only manifest-planning step to `storage.ts` that validates JSON shape and field types and returns each entry's raw `entry`/`package` strings without resolving them against the filesystem, sharing its shape rules with `resolveMarketplaceManifest` rather than duplicating them
  ...
  - [ ] After a successful reduced attempt, use the shape-only planning step to compute the directories each `entry`/`package` names, extend the checkout to include them, then validate via `storage.ts`'s existing, unmodified `resolveMarketplaceManifest`
...
- [ ] Re-derive the footprint on update (PR #2)
  - [ ] In `updater.ts`, read the target commit's manifest content before `moveCheckout` runs, without requiring that commit's complete tree to already be present, and run it through the same shape-only planning step PR #1 adds
