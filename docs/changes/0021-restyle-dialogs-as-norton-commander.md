...
- **Tests assert on plain text, and styling is forced on.** Rendered output now carries escape sequences. The dialog tests gain a helper that strips them before matching and a helper that returns the text of the inverted row, and every existing assertion on `> label` markers is rewritten against the inverted row; a test that today passes because a marker is absent is rewritten to assert which row is inverted instead, so it cannot pass vacuously. The renderer's styling library decides at load time whether standard output is a terminal and emits no sequences under a piped test run, so the test run MUST have styling forced on before any production module loads, or the inverted-row helper and the shading test are no-ops that fail; how it is forced is a design decision below.
...
- **Decision:** Tests strip escape sequences and assert on text, locate the active option through an inverted-row helper, and force styling on through a test preload.
  ...
  - **Mechanism:** a Bun test preload, registered in `bunfig.toml`, that sets `FORCE_COLOR=1` before any module loads; the styling library reads that variable ahead of its terminal check.
