...
The `update` plugin claims the `update` namespace and puts its behavior on the namespace root: a `--dry-run` flag, optional positional item names, and an action. It calls `api.updaters()`, gathers from each participant in order, prints a line per item, and — unless the run is a dry run — applies each in-scope item, printing its result.

Failure reaches it two ways and it handles both without knowing what failed. A participant that throws is caught, so one throw becomes one reported failure rather than an aborted run. An item that comes back carrying a failure is reported as failed and never applied, while the items beside it — from the same participant — are applied normally. Either one sets the flag that decides the exit code. Its only knowledge of what it is updating remains the strings on an item.
