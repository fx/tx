...
- **Decision:** Treat `GIT_SSH_COMMAND`, `GIT_SSH`, and `core.sshCommand` as one question — is an SSH command configured — and inject nothing when any of them answers yes, reading `core.sshCommand` from the global and system scopes only. Supersedes an earlier decision on this branch that checked `GIT_SSH_COMMAND` alone, and a second that read `core.sshCommand` without restricting its scope.
  ...
  - **Alternatives considered:** Appending the option was rejected as invention. Overriding outright was rejected as breaking the case the retry exists to serve. Checking only the environment was rejected once `core.sshCommand` turned out to be the ordinary way of configuring a deploy key. Reading every scope at once was rejected once the local scope turned out to describe a command the clone cannot use.
...
- **Decision:** Compose the attempt names credential-free instead of scrubbing them, and scrub the quoted Git output by whole URL form — the source as it was handed to Git and the same source with its password component dropped, both taken from the raw source string, plus the raw password alone as a safety net. Never remove a bare username. Supersedes two earlier decisions on this branch: one that redacted only the two attempt names, and one that removed literals taken from the parsed `URL` — among them the bare username — from the composed message as a whole.
  - **Why:** Redacting the names alone leaves the leak in place, because the message inlines each underlying Git message and Git echoes the clone URL in its stderr with only the password stripped: `fatal: unable to access 'https://<token>@github.com/owner/repository.git/': …`. The CLI writes `error.message` to standard error, so the token reaches the terminal and the CI log through the quoted output rather than through the label. But removing literals taken from the parse fails on both counts. `URL` percent-encodes what it parses while Git is handed the raw string and echoes it back raw, so a password holding any character `URL` escapes — `@`, a space, `:`, `/`, `?`, `#` — yields literals that match nothing and the secret survives verbatim. And deleting the bare username rewrites unrelated text: `git`, `me`, and `com` are all real HTTP(S) usernames, so `https://git:token@github.com/acme/tools.git` was reported as `hub.com/acme/tools.`, a repository nobody typed.

    Both failure modes come from treating a credential as a substring of a sentence. So the names are built rather than repaired — the source parsed, its userinfo cleared, its `href` taken — and the Git output is scrubbed before it is inlined, which puts every name out of reach of every replacement. What the scrub removes is a whole URL, because a whole URL is what Git repeats: anchored that way it cannot collide with host or path text, and reading the userinfo from the raw source string rather than from the parse — the authority between `://` and the first `/`, `?`, or `#`, its userinfo everything before the last `@`, its password everything after the first `:` — keeps the literal spelled exactly as Git will spell it. The bare password follows as rule three, for output that quotes it outside a URL. The username stays because it is an identifier rather than the secret, and the accepted cost is stated plainly: nothing removes a `user@` run on its own any more, so a password-stripped URL Git quotes in some third form may keep its username. That is strictly better than corrupting the message around it. The derived SSH candidate is `git@host:path` and carries no credential at all, which is why nothing scrubs it.
...
- [ ] Specify the SSH retry in [Plugin System: Marketplace Plugin Ownership](../specs/plugin-system/index.md#marketplace-plugin-ownership)
  ...
  - [ ] State that Git's terminal prompt is disabled for every clone attempt, that credential helpers and askpass stay in effect, that every attempt runs in batch mode by default because Git can rewrite an HTTP(S) source to SSH before the first one dials, and that an SSH command configured through `GIT_SSH_COMMAND`, `GIT_SSH`, or `core.sshCommand` in a scope a clone applies is used as it stands and may still prompt, a repository-local `core.sshCommand` counting for nothing
...
- [ ] Implement the fallback in `plugins/marketplace/manager.ts`
  ...
  - [ ] Add a batch-mode SSH command to every clone attempt, settled once before the loop, and only when neither `GIT_SSH_COMMAND`, nor `GIT_SSH`, nor `core.sshCommand` in the global or system scope configures one, reading the last through the injected Git runner
...
- [ ] Cover the new behavior in `test/marketplaces.test.ts`
  ...
  - [ ] Add tests that a configured `GIT_SSH_COMMAND`, `GIT_SSH`, or `core.sshCommand` suppresses the batch-mode default, that nothing configured produces it on every attempt, that the probe reads `--global` and `--system` and no other scope, and that no probe runs at all when an environment variable already answers
