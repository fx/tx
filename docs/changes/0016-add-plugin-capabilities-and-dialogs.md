...
The dialogs provider registers one local structural service under `dialogs`. Its `select` implementation renders through the injected terminal streams, resolves the selected opaque value or `undefined` for cancellation, and completes renderer cleanup before settling. Normal cleanup restores the terminal's prior input state; if an injected raw-mode disable or unref method persistently throws, the provider retries finitely and rejects with the first cleanup failure because restoration through that method is impossible. A controlled test consumer proves that the registry crosses plugin boundaries without creating a production namespace solely for demonstration.
