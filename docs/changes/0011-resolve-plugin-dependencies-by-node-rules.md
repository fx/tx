...
- Every way a bundle can report failure MUST name that specifier, including the ways `Bun.build` does not produce under the options used here. It throws for a failure today, but that is a default it has changed before, and a build whose reported failure went unread would surface as a plugin exporting nothing — a message describing neither the problem nor where it is. The bundler MUST therefore be injectable so those outcomes can be driven directly.
...
**Every reported failure names the specifier.** `Bun.build` throws for a failure under these options, so the unsuccessful-result path is unreachable through them today. It is handled regardless: `throw` is a default Bun has changed once already — this repository's own `build.ts` still checks `success` — and the cost of not handling it is an empty bundle reported as "must default-export a function", which points at the wrong thing in precisely the case this change exists to diagnose. The bundler is a parameter so those outcomes can be tested directly rather than left to a future Bun to discover.

**No splitting, enforced by the type rather than by prose.** Split output references sibling chunks by relative path, and this loader writes nothing to disk for those references to find; serving them from memory does not work either, because a literal relative specifier is resolved before any plugin hook runs. `splitting: false` is passed explicitly and typed as that literal, so enabling it is a type error rather than a baffling runtime failure.
